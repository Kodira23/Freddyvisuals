...
// Placeholder data (all prices in KSH)
const PLACEHOLDER_SERVICES = [
  ...
  },
  {
    ...
    price_from: 9000,
    duration: 'By Package',
    image_url: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=900&q=80', // Placeholder image; admin can replace with video
      ...
      {svc && (
        <div className="service-detail container" key={svc.id}>
          <div className="service-detail__image">
            {isVideoUrl(svc.image_url) ? (
              <video controls style={{ width: '100%', height: 'auto', maxHeight: '500px' }}>
                <source src={svc.image_url} type={`video/${svc.image_url.split('.').pop()}`} />
                  ...
                  <div className="service-card__img">
                    {isVideoUrl(s.image_url) ? (
                      <video style={{ width: '100%', height: '100%', objectFit: 'cover' }}>
                        <source src={s.image_url} type={`video/${s.image_url.split('.').pop()}`} />
                      </video>
                    ) : (
